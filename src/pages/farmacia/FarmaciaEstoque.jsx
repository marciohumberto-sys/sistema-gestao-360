...
import { mockCategorias } from '../../mocks/farmaciaMocks';
...
const FarmaciaEstoque = () => {
    const { estoqueLocal, unidadeAtiva, setOpenModal } = useFarmacia();
    const [animated, setAnimated] = useState(false);
    
    // Inicia a animação de preenchimento após a montagem do componente
    useEffect(() => {
    ...
    const METRICAS = useMemo(() => ({
        totalItens: estoqueLocal.length,
        semEstoque: estoqueLocal.filter(i => i.estoqueAtual === 0).length,
        abaixoMinimo: estoqueLocal.filter(i => i.estoqueAtual > 0 && i.estoqueAtual < i.estoqueMinimo).length,
        vencendoBreve: estoqueLocal.filter(i => {
    ...
    const itensFiltrados = useMemo(() => {
        return estoqueLocal.filter(item => {
            const matchBusca =
                busca === '' ||
                item.descricao.toLowerCase().includes(busca.toLowerCase()) ||
                item.codigo.toLowerCase().includes(busca.toLowerCase());
            
            const matchUnidade = unidadeFiltro === 'Todos' || item.setor === unidadeFiltro;

            let matchStatus = true;
            if (statusFiltro !== 'Todos') {
                if (statusFiltro === 'CRITICO') {
                    matchStatus = item.estoqueAtual < item.estoqueMinimo || item.estoqueAtual === 0;
                } else if (statusFiltro === 'SEM_ESTOQUE') {
                    matchStatus = item.estoqueAtual === 0;
                } else if (statusFiltro === 'ABAIXO_MINIMO') {
                    matchStatus = item.estoqueAtual > 0 && item.estoqueAtual < item.estoqueMinimo;
                } else {
        ...
        if (sortFiltro) {
            ...
            result.sort((a, b) => {
                ...
                // Tratar data de validade que pode ser '-'
                if (key === 'validade') {
        ...
        return result;
    }, [busca, unidadeFiltro, statusFiltro, sortFiltro, estoqueLocal]);
            ...
            <div className="farmacia-card" style={{ padding: '1rem 1.25rem', gap: '0' }}>
                <div className="farmacia-toolbar" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-muted)', fontSize: '0.85rem', fontWeight: 500, background: 'var(--bg-body)', padding: '6px 12px', borderRadius: '8px', minWidth: 'fit-content' }}>
                        <span>{itensFiltrados.length} resultados (de {estoqueLocal.length})</span>
                    </div>
                </div>
            </div>
                        ...
                        <div style={{ padding: '1.5rem' }}>
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'var(--bg-body)', borderRadius: '6px', border: '1px solid var(--border)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'rgba(34, 197, 94, 0.1)', color: '#16a34a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Plus size={14} />
                                            </div>
                                            <div>
                                                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)' }}>Entrada de Estoque</div>
                                                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Lote: L002-A • NF: 99812</div>
                                            </div>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#16a34a' }}>+500 {itemFoco.unidade}</div>
                                            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Hoje, 09:15</div>
                                        </div>
                                    </div>

                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: 'var(--bg-body)', borderRadius: '6px', border: '1px solid var(--border)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.1)', color: '#dc2626', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                <Minus size={14} />
                                            </div>
                                            <div>
                                                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)' }}>Saída de Estoque</div>
                                                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Setor: Internação • Req: 394</div>
                                            </div>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#dc2626' }}>-20 {itemFoco.unidade}</div>
                                            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>Ontem, 14:30</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
