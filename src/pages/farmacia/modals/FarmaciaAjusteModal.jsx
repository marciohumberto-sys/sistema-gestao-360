...
const FarmaciaAjusteModal = ({ isOpen, onClose }) => {
    ...
    useEffect(() => { if (isOpen) { setForm(def); setMed(null); } }, [isOpen]);
    ...
    // Auto-fill estoque da unidade ativa (ajuste 3)
    const handleSelectMed = item => {
        setMed(item);
        if (item) {
            const uid = unidadeAtiva?.id || 'upa';
            const itemLocal = estoqueLocal.find(i => i.codigo === item.codigo);
            const estoqueUnidade = itemLocal?.estoquePorUnidade?.[uid] ?? itemLocal?.estoqueAtual ?? '';
            setForm(prev => ({ ...prev, quantidadeAnterior: String(estoqueUnidade) }));
        } else {
            ...
            <div className="farmacia-modal">
                    ...
                    <div className="farmacia-modal-grid">
                        <div className="farmacia-form-group col-span-2">
                            ...
                            {med && (
                                <div style={{ 
                                    ...
                                    display: 'flex', flexWrap: 'wrap', gap: '1rem', color: 'var(--text-muted)'
                                }}>
                                    <div><span style={{ fontWeight: 500 }}>Estoque atual:</span> <strong style={{ color: 'var(--color-secondary)' }}>{form.quantidadeAnterior || '0'}</strong></div>
                        ...
                        <div className="farmacia-form-group col-span-1" style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            <div style={{
                                width: '100%',
                                padding: '10px 12px',
                                background: isNeutral ? 'transparent' : (diff > 0 ? 'rgba(0, 150, 125, 0.05)' : 'rgba(239, 68, 68, 0.05)'),
                ...
                <div className="farmacia-modal-footer">
                    ...
                    <button 
                        ...
                        disabled={!med || !form.quantidadeAjustada || !form.motivo}
