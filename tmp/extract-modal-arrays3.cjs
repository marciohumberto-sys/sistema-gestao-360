...
// Find the end of the Rateio content. It ends exactly at the `})()` call that closes the second IIFE loop for secretariats.
// Right after that, there are three `</div>` tags closing the structural paddings.
const innerEndIdx = lines.findIndex((l, i) => i > innerStartIdx && i < eIdx && l.includes('})()}'));
