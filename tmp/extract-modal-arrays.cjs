...
// The block to extract is between 761 and 961 (0-indexed lines 762 to 962)
// Actually let's use exact findIndex so it's immune to line number shifts
...
// inner rateio logic ends with </div> that closes the padding block
...
// We remove the entire TR block (from trStartIdx to trEndIdx)
lines.splice(trStartIdx, trEndIdx - trStartIdx + 1);

// Locate where to insert the new modal (right before {/* Modal Item */})
